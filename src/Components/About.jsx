...
const About = () => {
  return (
    <>
    <section className="w-full flex flex-col items-center justify-center py-12 px-4 md:px-16 bg-white text-center">
      {/* ABOUT US HEADER */}
      <h2 className="text-3xl text-black md:mt-[10vh] md:text-4xl font-extrabold mb-2 tracking-tight"
      style={{textShadow: "2px 2px 4px rgba(0, 0, 0, 0.5)",}}>
        ABOUT US
      </h2>
      <p className="text-sm md:text-base text-gray-600 mb-6">
        CipherEleven– Committed to Creating Successful <br /> Entrepreneurs and Brands!!!
      </p>
      ...
      <div className="relative bg-white w-full md:w-[80%] rounded-xl overflow-hidden shadow-2xl">
        ...
        <div className="absolute inset-0 bg-black bg-opacity-50 flex flex-col justify-center items-center px-6 text-white">
          ...
          <button className="bg-[#FF9800] hover:bg-[#e68900] hover:scale-110 text-white font-semibold py-2 px-5 rounded-md shadow-md transition-all">
            View Portfolio
          </button>
        </div>
      </div>
      ...
      <div className="max-w-4xl text-center">
        <h3 className="text-2xl text-black md:text-3xl font-bold mb-4"
        style={{textShadow: "2px 2px 4px rgba(0, 0, 0, 0.5)",}}>Why are we here ?</h3>
        <p className="text-gray-700 text-sm md:text-base leading-5.5 mb-4">
          ...
          strengthen, and transform their digital footprints in the competitive
          marketplace.
        </p>
        <p className="text-gray-700 text-sm md:text-base leading-5.5">
          ...
          demand generation, nurture their customer journey, improve conversion,
          sales & revenues, and gain a competitive edge.
        </p>
      </div>
    </section>
     ...
     <section className="w-full flex flex-col items-center justify-center bg-white">
      ...
      <div className="w-full bg-[#1F2732] text-white py-12 px-6 md:px-16 flex flex-col items-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 text-center max-w-6xl">
          {/* Cards  */}
          {data.StrategicCard.map(({id,image,title,description,accent})=>(
          <StrategCard key={id} image={image} title={title} description={description} accent={accent} />
          ))}
        </div>
      </div>
      ...
      <p className="text-sm md:text-base text-gray-600 mb-6">
         We believe in thinking outside the box. The Team comes up with a brand-new vision and creative solution to every new study of the requirement. We believe in thinking outside the box. The Team 
comes up with a brand-new vision and creative solution to every new study of the requirement
      </p>
      </div>
    </section>
    </>
